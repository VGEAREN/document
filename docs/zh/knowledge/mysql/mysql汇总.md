...
&emsp;&emsp;&emsp;<a href="#43">4.3.1. 脏读解决</a>  
&emsp;&emsp;&emsp;<a href="#44">4.3.2. 不可重复读问题出现</a>  
&emsp;&emsp;<a href="#45">4.4. RU读未提交（Read Uncommitted）</a>  
&emsp;&emsp;&emsp;<a href="#46">4.4.1. 脏读问题出现</a>  
&emsp;&emsp;<a href="#47">4.5. 多版本并发控制（Multi-Version Concurrency Control，MVCC）</a>  
&emsp;&emsp;&emsp;<a href="#48">4.5.1. 前置知识</a>  
&emsp;&emsp;&emsp;<a href="#49">4.5.2. MVCC原理</a>  
&emsp;&emsp;&emsp;&emsp;<a href="#50">4.5.2.1. 版本链</a>  
&emsp;&emsp;&emsp;&emsp;<a href="#51">4.5.2.2. undo log</a>  
&emsp;&emsp;&emsp;&emsp;<a href="#52">4.5.2.3. Read View(读视图)</a>  
&emsp;&emsp;&emsp;<a href="#53">4.5.3. 隔离级别与MVCC</a>  
&emsp;&emsp;&emsp;<a href="#54">4.5.4. 参考资料</a>  
...
&emsp;&emsp;<a href="#62">5.4. crash-safe的设置</a>  
...
&emsp;&emsp;&emsp;<a href="#126">10.8.1. 两种日志有以下三点不同</a>  
...
### <a name="11">增删操作与索引</a><a style="float:right;text-decoration:none;" href="#index">[Top]</a>
...
**解决空洞问题**，对表进行重建。
### <a name="12">索引规则</a><a style="float:right;text-decoration:none;" href="#index">[Top]</a>

#### <a name="13">覆盖索引</a><a style="float:right;text-decoration:none;" href="#index">[Top]</a>
InnoDB存储引擎支持覆盖索引，即从辅助索引中就可以得到查询的记录，而不需要查询聚集索引中的记录。
- 如： ``` select id, b from t where b = xxx   (id为主键，b为索引)```
  由于覆盖索引可以减少树的搜索次数（减少IO），显著提升查询性能，所以使用覆盖索引是一个常用的性能优化手段。

#### <a name="14">最左前缀原则</a><a style="float:right;text-decoration:none;" href="#index">[Top]</a>
B+树这种索引结构，可以利用索引的“最左前缀”，来定位记录。

#### <a name="15">索引下推</a><a style="float:right;text-decoration:none;" href="#index">[Top]</a>
``` 
...
### <a name="16">索引选择规则</a><a style="float:right;text-decoration:none;" href="#index">[Top]</a>
优化器选择索引的目的是找一个最优方案，用最小的代价执行语句。其中扫描行数、是否使用临时表、是否排序等因素都会影响优化器对索引的选择判断。


#### <a name="17">导致索引失效情况</a><a style="float:right;text-decoration:none;" href="#index">[Top]</a>
...
![image](./images/transactiongeli2.png)
...
![image](./images/transactiongeli4.png)
...
```
![image](./images/orderby1.jpg)
> Extra这个字段中的`Using filesort`表示的就是需要排序，MySQL会给每个线程分配一块内存用于排序，称为sort_buffer。

![image](./images/orderby2.jpg)
这个语句执行流程如下所示 ：
...
![image](./images/orderTmpMemoryTable.png)
...
## <a name="105">MySQL架构</a><a style="float:right;text-decoration:none;" href="#index">[Top]</a>

### <a name="106">主从模式(读写分离)</a><a style="float:right;text-decoration:none;" href="#index">[Top]</a>
读写分离的主要目标就是分摊主库的压力

客户端直连

![image](./images/readwrite1.png)
...
### <a name="107">分库分表</a><a style="float:right;text-decoration:none;" href="#index">[Top]</a>

#### <a name="108">分库分表场景</a><a style="float:right;text-decoration:none;" href="#index">[Top]</a>
由于数据量过大而导致数据库性能降低的问题，将原来独立的数据库拆分成若干数据库组成 ，将数据大表拆分成若干数据表组成，使得单一数据库、单一数据表的数据量变小，从而达到提升数据库性能的目的。

#### <a name="109">垂直切分</a><a style="float:right;text-decoration:none;" href="#index">[Top]</a>
...
#### <a name="110">水平切分</a><a style="float:right;text-decoration:none;" href="#index">[Top]</a>
...
### <a name="112">MGR(MySQL Group Replication)</a><a style="float:right;text-decoration:none;" href="#index">[Top]</a>
...
### <a name="113">主从数据同步</a><a style="float:right;text-decoration:none;" href="#index">[Top]</a>

参考资料：[MySQL5.7新特性--官方高可用方案MGR介绍](https://www.cnblogs.com/luoahong/articles/8043035.html)
#### <a name="114">MySQL异步复制</a><a style="float:right;text-decoration:none;" href="#index">[Top]</a>
...
#### <a name="115">MySQL半同步复制</a><a style="float:right;text-decoration:none;" href="#index">[Top]</a>
在master事务的commit之前，必须确保一个slave收到relay log并且响应给master以后，才能进行事务的commit。但是slave对于relay log的应用仍然是异步进行的

#### <a name="116">MySQL组复制</a><a style="float:right;text-decoration:none;" href="#index">[Top]</a>
基于传统异步复制和半同步复制的缺陷——数据的一致性问题无法保证

## <a name="117">其他面试题</a><a style="float:right;text-decoration:none;" href="#index">[Top]</a>

### <a name="118">主键索引是否使用自增</a><a style="float:right;text-decoration:none;" href="#index">[Top]</a>
使用自增好处：
...
### <a name="119">临时表与内存表</a><a style="float:right;text-decoration:none;" href="#index">[Top]</a>
内存表，指的是使用Memory引擎的表
...
### <a name="120">重建索引是否合理</a><a style="float:right;text-decoration:none;" href="#index">[Top]</a>
...
### <a name="121">mysql数据库抖动</a><a style="float:right;text-decoration:none;" href="#index">[Top]</a>
...
### <a name="122">读已提交和可重复读是如何实现的</a><a style="float:right;text-decoration:none;" href="#index">[Top]</a>
1. ReadView：\
...
### <a name="123">读已提交和可重复读区别？</a><a style="float:right;text-decoration:none;" href="#index">[Top]</a>
举两个事务线程的例子。

### <a name="124">数据库数据库一致性是如何实现的？</a><a style="float:right;text-decoration:none;" href="#index">[Top]</a>
通过redoLog 与binlog 两阶段提交 保证事务一致性。

### <a name="125">redoLog、undoLog、binlog区别？</a><a style="float:right;text-decoration:none;" href="#index">[Top]</a>
...
#### <a name="126">两种日志有以下三点不同</a><a style="float:right;text-decoration:none;" href="#index">[Top]</a>
...
### <a name="127">一个事务开启什么时候产生三种日志？</a><a style="float:right;text-decoration:none;" href="#index">[Top]</a>
1. 事务开启
...
### <a name="128">可重复读隔离级别下，事务中select一条记录巨慢</a><a style="float:right;text-decoration:none;" href="#index">[Top]</a>
session A：
```
start transaction;
// 时刻A
...
### <a name="129">数据库什么情况会出现死锁？如何处理死锁？</a><a style="float:right;text-decoration:none;" href="#index">[Top]</a>
...
#### <a name="130">由于索引导致的死锁</a><a style="float:right;text-decoration:none;" href="#index">[Top]</a>
...
### <a name="131">如何优化SQL</a><a style="float:right;text-decoration:none;" href="#index">[Top]</a>
...
#### <a name="132">order by 优化</a><a style="float:right;text-decoration:none;" href="#index">[Top]</a>
Order by查询的两种情况：
...
#### <a name="133">sub-query 子查询</a><a style="float:right;text-decoration:none;" href="#index">[Top]</a>
对于子查询，一般的优化策略是做等价改写
...
#### <a name="134">limit 优化 - 延迟关联</a><a style="float:right;text-decoration:none;" href="#index">[Top]</a>
...
##### <a name="135">相关资料</a><a style="float:right;text-decoration:none;" href="#index">[Top]</a>
...
#### <a name="136">Or/And Condition</a><a style="float:right;text-decoration:none;" href="#index">[Top]</a>
处理策略
1. And子句多个条件中拥有一个过滤性较高的索引即可;
2. Or条件前后字段均要创建索引;
3. 为最常用的And组合条件创建复合索引。

#### <a name="137">join 优化</a><a style="float:right;text-decoration:none;" href="#index">[Top]</a>
...
### <a name="138">数据库连接池、数据库连接线程安全的吗？</a><a style="float:right;text-decoration:none;" href="#index">[Top]</a>
...
### <a name="139">InnoDB 和 MyIsam 数据库引擎的区别</a><a style="float:right;text-decoration:none;" href="#index">[Top]</a>
...
### <a name="140">MySQL数据页(储存页)</a><a style="float:right;text-decoration:none;" href="#index">[Top]</a>
...
### <a name="141">高并发数据库读写压力大怎么处理？</a><a style="float:right;text-decoration:none;" href="#index">[Top]</a>
...
### <a name="142">其他</a><a style="float:right;text-decoration:none;" href="#index">[Top]</a>
#### <a name="143">什么是覆盖索引跟回表？</a><a style="float:right;text-decoration:none;" href="#index">[Top]</a>
见索引规则

#### <a name="144">`left join,right join,inner join`,表表关联什么区别？</a><a style="float:right;text-decoration:none;" href="#index">[Top]</a>
...
#### <a name="145">大事务会有什么影响？</a><a style="float:right;text-decoration:none;" href="#index">[Top]</a>
